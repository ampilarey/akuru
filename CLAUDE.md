...
| `docs/BOOKSHOP_PLAN.md` | Akuru Online Store (renamed from Bookshop 2026-09-26; multi-vendor books and educational items) plan: slices B0–B9, the owner's decisions (all made 2026-09-25) and the audit |
...
7. **Hifz is frozen** until its §2b migration phase: namespace/route changes only; no behavior change, no refactor, no dashboard redesign. Freeze is **scope discipline** (do not refactor while moving), not production-safety — no live Hifz users yet (ADR-021). **This rule's own condition is met: §2b completed 2026-09-12 (F5, ADR-029), so the freeze has expired by its own terms.** What survives of the Blade app — hub, five dashboards, programmes, enrolments, milestones, reports — is ordinary code again. Retiring those screens is still a separate IA decision with its own parity work, not something to do "while you're there" (rule 1).
...
- **Merge gates (per ROADMAP §4):** one slice per PR; a required CI check reporting `success` on the PR head before merge; no direct pushes to `main`; a spec-mandated verification script's captured output must be in STATUS.md **before** the deploy it gates. A gate whose evidence isn't recorded has not run. **These are currently discipline, not mechanism.** This line used to say "`main` is branch-protected"; it is not, and never has been — checked 2026-09-15, `"protected": false` on the branches API, and a PR's `mergeable_state` reading `unstable` rather than `blocked` with CI still running (STATUS §5ep, `docs/BRANCH_PROTECTION.md`). So nothing stops a direct push, a force push, or a merge over red CI except whoever is at the keyboard. Follow the gates anyway — and treat ADR-027's *read the conclusion back, never assume it* as load-bearing rather than belt-and-braces, because it is the only check there is. Restoring the mechanism needs a repo admin; until then, do not relax the rule on the grounds that nothing is enforcing it.
...
code + tests green + arch tests green + **walked in a browser** (a user
can complete the task in a browser, not only when tests pass) +
STATUS.md updated + (if a decision was made) ADR written + no
out-of-scope files touched.

This is the lesson of pilot rehearsal rounds 1 and 2: CI-green slices
still left empty grids, live SMS, and Blade landings that hid the work.
