...
| 3 C1 course certificates | Yes. `certificate_templates` + `issued_certificates`; admin builder; issue; public QR verify. | `CourseCertificateTest`. | Walked this PR: template → issue → unauthenticated `/verify/certificates/{ulid}` face only; CSV. | Unlocalized verify URL. Morph aliases `certificate_template`, `issued_certificate`. HTML, not PDF. |
