...
| S3.1 grading foundations | Yes. Scales, types, weights UI. | `GradingFoundationsTest` including HTTP store. | Weights form saves a year scheme (#96): numeric defaults summing to 100. Walked 2026-09-22 (`exams.mjs`, §5ex): the scheme is saved through the form and the report cards it feeds are read back by a family. | Previously walked **fail** (R2/R3 JSON zeros), fixed in #96. |
...
| S4.2 fee structures | Yes. | `FeeStructureTest`. | Walked 2026-09-23 (`fees.mjs`, §5fa): the office creates an active structure for a class through the form, and the invoice run bills from it. | Default seed includes pilot fees via `PilotRehearsalSeeder` (#87). |
...
| Arabic A.1–A.3 | Yes. Letters/harakas, skill tag, reports. | `ArabicReferenceTest`, `ArabicSkillActivityTest`, `ArabicSkillReportTest`. | **Walked 2026-09-23** (`arabic.mjs`, §5fl): the office adds a letter, the author tags a reading activity with it on the plain selection pattern, the engine scores the student 1/1 with no teacher and no AI, and both skill reports list it — the student's under *reading* with the attempt, the office's with the letter. **Run in the full sequence (§5fx) the student's report read only their latest enrolment, so enrolling on any second course zeroed every earlier attempt on it** — fixed; the report is now theirs across every course. | No AI (rule 8). **Audited 2026-08-27: PASS** — tables + `NormalizeTextAnswerAction` (spec normalization) + reports verified; skill metadata rides the four activity patterns (placement caveat = Phase 2 Components note). |
...
sequence `all.mjs` is for, 34/35: `arabic.mjs` failed its student-report
step — *SMOKE-Arabic-Activity reading 0* — while the office's report,
one step later, listed the same activity with its one attempt. Alone,
10/10. After `author`, `intake`, `learn`, `assess`, `certify` and `buy`,
9/10, every time.

**The defect.** `LearnArabicReportController` scoped the student's report
to `ResolveLatestEnrollmentIdAction` — their single most recent live
enrolment. A student's attempts live on the enrolment they were made
under, so the day a student enrols on any other course (here `buy.mjs`
sells them one after `arabic.mjs`'s attempt) every attempt on the earlier
course reads zero on their own report, while staff see it fine. Not a
walk-order artefact: a student on two courses is the ordinary case. Now
`ListLiveEnrollmentIdsAction` (engine seam, the plural of the latest-id
one) and `ListActivityAttemptsByIdsAction` take a list, and the report is
the student's across every course they hold. `ArabicSkillReportTest`
enrols on a second course after the attempt and reads the first back —
red without the controller change, green with it. The other consumer of
the latest-id seam, `SubmitRecitationAction`, *attaches* a submission to
an enrolment and is right to pick one.

**Two reds that were the room, not the code.** The queue worker had died
with the dev server and MariaDB earlier in the day; `exams.mjs` says
exactly that (*is a queue worker running?*) and passed 28/28 once it was
back. `register.mjs` failed once in the first full run and passed in the
second and alone; its output was lost to a `tail`, so it is recorded as
seen once, not reproduced, and the full-run log is now kept whole.

**Walked in a browser.** `arabic.mjs` 10/10 after the six walks that
precede it; the full sequence 35/35.
...
**Addendum, same day.** Three more rows were still behind the walks when
this section said "nothing else": 1A.2–1A.7 called activity and
assessment *authoring* unverified by walk, which `assess.mjs` (§5fi),
`quran.mjs` (§5fm) and `arabic.mjs` (§5fl) had already done; S3.1 did not
say `exams.mjs` (§5ex) saves the weight scheme through the form; S4.2
said its fee structure was "still seeded rather than created by hand",
which `fees.mjs` creates through the form. All three rows now say what
the walks do. Read back a second time, deliberately — the first reading
searched for the word *unverified* and missed two rows that said the
