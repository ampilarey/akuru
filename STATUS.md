...
**Verified against:** `main` after PRs **#86–#118**.  
...
| W1.6 funnel measurement | Yes. `funnel_events` (no `academic_year_id` — website conversion, like `leads`); `course_view` / `register_click` / `registration_started` / `payment_completed` / `whatsapp_click` / `syllabus_download`; admin `/admin/public-site/funnel` + CSV. | `CourseFunnelTest`. | Walked **#117**: Arabic Beginners show writes `course_view` (3); sticky/hero Register beacon `register_click` (1); WhatsApp beacon `whatsapp_click` (1); checkout (logged-in continue) `registration_started` (1). Admin `/en/admin/public-site/funnel` lists the course, view→click **33.3%**, ADR-022 decision “Keep iterating W1 content…”. CSV `funnel.csv`. | Client may only post click names. `payment_completed` is webhook/`finalizeByReference` success only (never the return URL). Admissions/Finance pass **strings** into Website `RecordFunnelEventAction` (no Enum import). No new Spatie permission; no AppShell link. Iterate-from-data rule is ADR-022. |
...
| W2.2 daily content store | Yes. `daily_contents` (no `academic_year_id` — public-site calendar, like `leads`); ayah/hadith/saying/reminder; Blade `/admin/public-site/daily-content` calendar + form + queue + theme batch + CSV. | `DailyContentStoreTest`. | Walked this PR: admin saved Fātiḥah 1:1 ayah draft (live preview Pickthall EN + Dhivehi + fixture source); complete Bukhari 1 hadith draft; queue hid Approve for the creator. Supervisor Ibrahim published the hadith (`approved_by` user 6). CSV `daily-content.csv`. Theme batch 3 knowledge reminders 2026-09-01..03 still draft. | Hadith four-field gate + maker–checker (ADR-024). Website uses `QuranTextProviderInterface` only. No AppShell link. Permissions `daily_content.manage` / `daily_content.approve`. This VM’s supervisor needed a verified `user_contacts` row (UserSeeder already calls `EnsureVerifiedEmailContactAction` on fresh seed). |
