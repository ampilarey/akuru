...
**Verified against:** `main` after PRs **#86–#116**.  
...
| W1.6 funnel measurement | Yes. `funnel_events` (no `academic_year_id` — website conversion, like `leads`); `course_view` / `register_click` / `registration_started` / `payment_completed` / `whatsapp_click` / `syllabus_download`; admin `/admin/public-site/funnel` + CSV. | `CourseFunnelTest`. | Walked this PR: Arabic Beginners show writes `course_view`; sticky Register / WhatsApp beacons write `register_click` / `whatsapp_click`; checkout writes `registration_started`; admin funnel lists the course + ADR-022 decision; CSV `funnel.csv`. | Client may only post click names. `payment_completed` is webhook/`finalizeByReference` success only (never the return URL). Admissions/Finance pass **strings** into Website `RecordFunnelEventAction` (no Enum import). No new Spatie permission; no AppShell link. Iterate-from-data rule is ADR-022. |
...
Hifz behaviour frozen. Deploy 3 not executed. Track B leftovers B1–B4 merged (#102–#105). Phase 3 C1–C3 merged (#106–#108). D1–D3 portal composition merged (#109–#111). W1.1–W1.5 merged (#112–#116). W1.6 funnel measurement is this PR.
