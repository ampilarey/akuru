...
| W2.1 Quran translations | Yes. `quran_translations` on existing `quran_ayahs`; `QuranTextProviderInterface` (Hifz `ReadQuranTextAction`); `quran:import-translations`. | `QuranTextProviderTest`. | Walked this PR: import fixture 1:1 EN+DV onto Demo Madani Mushaf ayah 1:1; provider returns Arabic + Pickthall English + Dhivehi teaching gloss. | No parallel Quran tables. Fixture is **not** a published edition (ADR-023). Operator imports a licensed set. No Hifz dashboard change. No public widget yet (W2.3). |
