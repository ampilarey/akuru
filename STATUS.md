...
| D3 staff overview | Yes. `/portal/overview` + CSV. Reads Academics `ListUnfilledRegistersAction` (unfilled, fill rates, plan adherence) and ExamsGrades `ListExamsAction::ungraded`. Admin/headmaster `/dashboard` redirect. | `StaffOverviewTest`; `RoleLandingTest`. | UNVERIFIED until this PR’s browser walk. | No new tables. Portal new files import Actions only, no Models / no Hifz. AppShell Overview (wrap 83). Super_admin landing stays Blade. |
