...
| D1 composed parent/student home | Yes. `/portal/home` + CSV; parent/student `/dashboard` redirect. Reads Academics/ExamsGrades/Finance/Courses Actions and `StudentHifzSummaryReader`. | `PortalHomeTest`; `RoleLandingTest`. | Walked this PR: parent Hassan `/en/dashboard` → `/en/portal/home` with Fatima attendance/exams/invoices/courses/Hifz sections + CSV. Student landing same page. | Portal new files import no other-domain Models and no `App\Domains\Hifz\`. AppShell Home link (wrap 81). |
