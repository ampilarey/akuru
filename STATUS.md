...
**Verified against:** `main` after PRs **#86–#111**.  
...
| W1.1 conversion urgency | Yes. Public course cards/detail: seats thresholds, deadline badge, early-bird from `courses.meta`, waitlist → `contact_inquiries`. Expired open courses hidden from Open Courses. | `CourseConversionTest`; `PublicRouteNamesTest`. | UNVERIFIED until this PR’s browser walk. | Settings group `conversion`. Occupying enrollments = pending+active (same as checkout). No `course_type` branch. |
...
Hifz behaviour frozen. Deploy 3 not executed. Track B leftovers B1–B4 merged (#102–#105). Phase 3 C1–C3 merged (#106–#108). D1–D3 portal composition merged (#109–#111). W1.1 conversion urgency is this PR.
