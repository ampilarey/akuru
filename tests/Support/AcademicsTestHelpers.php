...
{
    // `subjects.code` is globally unique. See uniqueFixtureSuffix() for why
    // the previous `fake()->unique()->numerify('###')` guaranteed nothing.
    return Subject::query()->create([
        'school_id' => makeSchool()->id,
        'name' => 'Arabic',
        'code' => 'ARB'.uniqueFixtureSuffix(),
        'type' => 'Arabic',
        'is_active' => true,
    ]);
}
...
function makeRoomRow(?string $name = null): Room
{
    return Room::query()->create([
        'name' => $name ?? 'Room '.uniqueFixtureSuffix(),
        'type' => RoomType::Lab,
        'bookable' => true,
        'active' => true,
    ]);
