...
return [
    ...
    'POST shop/newsletter/unsubscribe/{token}' => 'B9c: the confirm button on that page (a mail scanner opening the link unsubscribes nobody). Verified by the same token; throttled.',
    'POST shop/{vendor}/newsletter' => 'B9c: sign up for a shop\'s news from its public page, with explicit consent; anonymous visitors are the audience. Throttled; it writes one row per shop and address and reveals nothing back.',
    'GET shop/suggest' => 'Search suggestions for the shop box (BOOKSHOP_PLAN B7): names of products for sale, categories and vendors matching the typed text, as JSON. Public catalogue content only; throttled. No per-person data.',
    ...
    'GET gift-cards' => 'Public site content (LIBRARY_PLAN §15.3): the gift card page is a shop window; buying on it needs a sign-in and is guarded in the POST handler.',
    'POST gift-cards' => 'GUARDED IN THE HANDLER: aborts 403 when there is no current user. The route list cannot show that, which is why it is written down here.',
    'GET gift-cards/return' => 'GUARDED IN THE HANDLER: aborts 403 when there is no current user, and shows only the buyer\'s own latest order — nothing from the query string. Display-only; the card is issued by the webhook.',
