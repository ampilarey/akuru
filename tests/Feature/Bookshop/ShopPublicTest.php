...
/**
 * BOOKSHOP_PLAN slice B1b: the public Akuru Online Bookshop — one catalogue
...
it('shows every vendor\'s products for sale in one shop, and nothing that is not for sale', function () {
    ...
    $home = shopGet(route('public.shop.index'))->assertOk()
        ->assertSee('Akuru Online Bookshop')
        ->assertSee('Sold by')
    ...
    $page = shopGet(route('public.shop.vendor', 'fitrah'))->assertOk()
        ->assertSee('at Akuru Online Bookshop')
...
it('speaks the visitor\'s language where the vendor gave one', function () {
    ...
    App::setLocale('dv');
    shopGet(route('public.shop.index'))->assertOk()->assertSee('ނަމާދު ކުރާ ފޮތި')->assertSee('English Only')->assertSee('ފޮތްފިހާރަ');
    App::setLocale('ar');
    shopGet(route('public.shop.product', 'prayer-mat'))->assertOk()->assertSee('سجادة صلاة')->assertSee('في متجر أكورو للكتب على الإنترنت');
});
