...
## DoD
- [x] Staff month-in-the-life works end to end: check-ins recorded → leave requested/approved (balance moves, substitution created) → permit-expiry alert fires → appraisal completed → payroll period run, reviewed, approved → payslip PDF (trilingual) downloaded by staff in Portal → bank CSV exported. *Walked by `scripts/smoke/hr.mjs` 2026-09-23 (33 steps, STATUS §5fd), which found the "substitution created" half silently not happening and fixed it. Two clauses stay open: the payroll steps run only where `PAYROLL_ENABLED` is on (line 64, the owner's), and the payslip is the generic document, not yet trilingual (S5 slice 2).*
