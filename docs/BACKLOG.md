...
| C7 | **Akuru Online Store** (renamed from Bookshop 2026-09-26; owner's brief, 2026-09-25): a multi-vendor shop for printed books and educational materials, invited vendors only at first, every vendor's items in the one shop, a page per vendor, subdomain later. **Planned in `docs/BOOKSHOP_PLAN.md`** (v2.1 audited: slices B0–B9, decisions §13, audit §14; first vendor kit `docs/vendors/FITRAH.md`). Parked inside it for "later, on request" (B9): public vendor onboarding, custom hosts, cash on delivery, newsletter section, abandoned-cart reminders, bulk quotes for schools, USD pricing, a search service, analytics funnel. B0, B1a and B1b built (STATUS §5gu–§5gw); B2 next. | Owner's brief in conversation; `docs/BOOKSHOP_PLAN.md`. |
