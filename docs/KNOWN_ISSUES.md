...
| Was | Fix | Evidence |
|---|---|---|
| People → Students search/CSV only; no create | **this PR** `SaveStudentAction` + status via `ChangeStudentStatusAction`; guardian pivot; course-only nullables | `StudentDirectoryCrudTest`; browser: add child → roster picker |
