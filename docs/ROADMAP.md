...
> **As built (Phase F, 2026-08-27 — #131–#134, ADR-025).** F0 created `Components/{Arabic,Quran}` and moved the A-track classes in; F1 shipped the halaqa mirror gate (`halaqa:verify-mirror`); F2 mapped programs→Course(`hifz`)+Offering, enrollments, sessions and attendance with `halaqa:backfill-structure` / `halaqa:verify-structure`, and bound milestone completion through the ADR-022 evaluator contract; F3 landed §52.19–52.22 engine-keyed (submissions, haraka-strict mistake marks, revision schedules, memorization progress); F4 shipped the non-AI student/teacher/oversight dashboards. **Not done, gated (ADR-025):** manual recording mode (§52.9 — needs private media streaming), and formal retirement — the frozen Blade app remains the only UI for three-lane session-record entry, assignments (§52.18) and the milestone approval workflow, so the Quran dataset models + reader implementations move in the same future slice that deletes the Blade app, never before. The two-live-systems window this opens is accepted because rule 7's freeze keeps the Blade side inert.
