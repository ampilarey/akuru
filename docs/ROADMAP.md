...
**Gates are mechanism, not prose (lesson of 2026-08):** the S1 rollout showed that gates written as instructions get skipped under momentum — a four-slice PR self-merged in two minutes with CI running only post-merge, and Deploy 2 shipped before its mandatory verification script ever ran. Therefore: `main` carries branch protection (required CI check pre-merge, no direct pushes); one slice per PR; and any spec-mandated verification script (`morph-map:verify`, `students:verify-unification`, and successors) must have its **captured output recorded in STATUS.md before the deploy it gates**, not after. A gate whose evidence isn't in STATUS.md has not run.

**Who presses merge (ADR-027, 2026-08-29):** the original rule read "no bot self-merge". The owner now authorises agents to merge, so the gate is the *condition* rather than the actor: an agent may merge its own PR once the required CI check has **reported `success` on the PR head** — read the conclusion back, never assume it — and the PR is one slice with any gating evidence already in STATUS.md. Merging with CI queued, running, or failed stays prohibited, as does weakening branch protection to force a merge. This keeps the half of the S1 lesson that mattered: what went wrong there was four slices merging before any check reported, not the identity of whoever clicked.
...
> **As built (Phase F, 2026-08-27 — #131–#134, ADR-025).** F0 created `Components/{Arabic,Quran}` and moved the A-track classes in; F1 shipped the halaqa mirror gate (`halaqa:verify-mirror`); F2 mapped programs→Course(`hifz`)+Offering, enrollments, sessions and attendance with `halaqa:backfill-structure` / `halaqa:verify-structure`, and bound milestone completion through the ADR-022 evaluator contract; F3 landed §52.19–52.22 engine-keyed (submissions, haraka-strict mistake marks, revision schedules, memorization progress); F4 shipped the non-AI student/teacher/oversight dashboards. **Completed 2026-09-12 (#136–#138, #259, #267; ADR-029).** F5-P1/P2/P3 built the three parity workflows the gate named — three-lane session-record entry, §52.18 assignments, milestone approval — on the engine. §52.9's stated blocker turned out not to exist: private media streaming already shipped, so manual recording mode needed only replay-before-submit. F5 retirement then moved the Qur'an dataset and its readers into `Courses/Components/Quran`, deleted the four Blade controllers that read them, and **ported** a fourth workflow ADR-025 never listed — mushaf editorial, which had no engine equivalent and would otherwise have been lost. Rule 7's freeze ends here. What remains of the Blade app (hub, five dashboards, programmes, enrolments, milestones, reports) touches no dataset model; retiring it is an IA decision, not a dataset one.
...
- **Live data (ADR-021):** never drop `courses` columns, `registration_students`, or old quiz tables until migrated data is verified. Use additive migrations + backfill + verify + remove (3 separate deploys). **Until first real use** (no production system as of 2026-08-25; `test.akuru.edu.mv` is synthetic-only) dual-write windows and “≥2 weeks stable” waits are optional; additive migrations remain good practice. Rule 9 reactivates in full at first real student, payment, or Hifz user. The production-copy unification gate reactivates that same day.
- **Hifz untouched rule:** freeze until §2b — namespace/route changes only; no behavior change, no refactor, no dashboard redesign. This is **scope discipline** (do not refactor the module while moving it), not production-safety: Hifz is **not** in active use; there are no live Hifz users (ADR-021). Engine migration still happens only after Phase 2 (§2b).
...
- **Dual payment path is a standing risk until spec Phase 4.** Enrollment/payment code has two read paths (legacy BML checkout + engine). Treat it like the Hifz-frozen rule: any slice touching enrollment or payment reads must verify **both** paths. Dual-written columns follow rule 9 in full **once real payments exist** (ADR-021); until then there is no live money on either path. The 2026-08 incidents (morph-map FQCN drift; Deploy 2 shipped before `students:verify-unification` ran) were both this class of bug — divergence between a legacy path and a new path that nothing was checking.
