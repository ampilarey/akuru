...
| B9 | **Later, on request** — public vendor onboarding (apply → approve), custom host per vendor, whole-shop subdomain, cash on delivery, newsletter section, abandoned-cart reminders, bulk quotes for schools (B2B), USD pricing, search service, analytics funnel. **Requested 2026-09-26 ("B9"); built as sub-slices, one PR each.** B9a public vendor onboarding **built (STATUS §5hg)**; B9b cash on delivery **built (§5hh)** — the shop's own delivery or collection only, paid when the shop takes the cash, the commission owed back through payouts; B9c newsletter section and abandoned-cart reminders **built (§5hi)** — the shop sends its own newsletters from the exported list. | parked in `BACKLOG.md`; being built |
