...
| B11 | **The audit's seven (2026-09-26)** — §15 findings 9–14 and 16 as one slice (STATUS §5hr): the e-book link, a gift message, the returns rate, the bookstore open/closed switch, GST collected on the office report, alt text asked for on every photo, the gallery lightbox. **Built 2026-09-26.** | a shop ties a book to its e-book and words its photos; a customer sends a gift message; the office closes the bookstore for a day — walked, `followups.mjs` 22/22 |
...
| 9 | §4 **"read the e-book" link** for a product tied to a library item: `products.library_item_id` exists (B1a) but nothing writes it (no field in the product form) and nothing reads it (no link on the product page). | gap | **Built in B11** (STATUS §5hr): a select in the product form through `Library`'s `ListLibraryItemsAction`, the link on the product page. |
| 10 | §4 **gift message** at checkout: not built (order notes are). | gap | **Built in B11**: `gift_message` on the checkout and each order; on the order page, the order card and the packing slip. |
| 11 | §5 **returns rate** in the vendor's reports: refunded amounts are reported (B6), the rate is not. | gap | **Built in B11**: returned ÷ delivered orders on the money page. |
| 12 | §7 **shop on/off** switch for the office: not built. Holiday mode is per shop; the office can suspend a shop but not close the whole shop for a day. | gap | **Built in B11**: a switch with a notice on `/admin/bookshop`; `EnsureBookstoreOpen` answers the public shop routes with the notice (503) while customers' own orders, quotes and wishlist stay open. |
| 13 | §7 **GST collected** report: the office's tax report covers Akuru's commission and its GST (B6); GST on the shops' own sales is each registered shop's to file and is on its statement, not summed across shops. | gap (partial) | **Built in B11**: a summed `sales_gst` column on the office's tax report and its CSV. |
| 14 | §10 **alt text required on product images**: stored and rendered (`alt_text`, falling back to the title) but never asked for in the product form. | gap (accessibility) | **Built in B11**: a field on each photo in the product form, required on save; the title remains the fallback for a photo just uploaded. |
...
| 16 | §4 **gallery zoom**: a gallery with a large image and thumbnails; no zoom. | note | **Built in B11**: a `<dialog>` lightbox with previous/next and a further zoom; the links still open the large image without JavaScript. |
