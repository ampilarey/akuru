...
**Do this before anything below.** Nineteen scripted walks drive real browsers
...
structure to a receipt a parent opens, and the whole thing at phone width —
and they take about fifteen minutes against a host, unattended.
...
**Fifteen of the nineteen write data** — they submit absence notes, request
that children be collected, book meetings, enrol people, hand in recordings
for a teacher to judge, publish exam results to families and issue them
invoices. Against a host whose name does not look synthetic the
...
fifteen that change data. The `exams` walk needs a queue worker on the host
