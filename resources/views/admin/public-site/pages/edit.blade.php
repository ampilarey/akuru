...
<div class="container mx-auto px-4 py-8">
    <div class="mb-6">
        ...
        <h1 class="text-3xl font-bold text-gray-900 mt-2">Edit Page: {{ $page->title }}</h1>
    </div>
    ...
    <div class="card">
        <form action="{{ route('admin.pages.update', $page) }}" method="POST" class="p-6 space-y-6">
            ...
            <div>
                ...
                <textarea name="excerpt" id="excerpt" rows="2" class="form-input w-full rounded-md @error('excerpt') border-red-500 @enderror">{{ old('excerpt', $page->excerpt) }}</textarea>
                @error('excerpt')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="body" id="body" rows="12" required class="form-input w-full rounded-md @error('body') border-red-500 @enderror">{{ old('body', $page->body) }}</textarea>
                @error('body')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="cover_image" id="cover_image" value="{{ old('cover_image', $page->cover_image) }}"
                       class="form-input w-full rounded-md @error('cover_image') border-red-500 @enderror">
                @error('cover_image')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
