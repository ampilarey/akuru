...
<div class="container mx-auto px-4 py-8">
    <div class="mb-6">
        ...
        <h1 class="text-3xl font-bold text-gray-900 mt-2">Edit Course: {{ $course->title }}</h1>
    </div>
    ...
    <div class="card">
        <form action="{{ route('admin.courses.update', $course) }}" method="POST" class="p-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $course->title) }}" required class="form-input w-full rounded-md @error('title') border-red-500 @enderror">
                @error('title')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="slug" id="slug" value="{{ old('slug', $course->slug) }}" required class="form-input w-full rounded-md @error('slug') border-red-500 @enderror">
                @error('slug')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="short_desc" id="short_desc" rows="2" required class="form-input w-full rounded-md @error('short_desc') border-red-500 @enderror">{{ old('short_desc', $course->short_desc) }}</textarea>
                @error('short_desc')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="body" id="body" rows="8" required class="form-input w-full rounded-md @error('body') border-red-500 @enderror">{{ old('body', $course->body) }}</textarea>
                @error('body')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            @include('admin.public-site.courses._cta_fields', ['course' => $course])
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="cover_image" id="cover_image" value="{{ old('cover_image', $course->cover_image) }}" required class="form-input w-full rounded-md @error('cover_image') border-red-500 @enderror">
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="fee" id="fee" value="{{ old('fee', $course->fee) }}" min="0" step="0.01" class="form-input w-full rounded-md">
                </div>
                <div>
                    <label for="seats" class="block text-sm font-medium text-gray-700 mb-1">Seats</label>
                    <input type="number" name="seats" id="seats" value="{{ old('seats', $course->seats) }}" min="1" class="form-input w-full rounded-md">
                </div>
            </div>
