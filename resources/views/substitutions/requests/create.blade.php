    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-gray-900">
                {{ __('New Substitution Request') }}
            </h1>
            <p class="mt-2 text-gray-600">
                {{ __('Create a new substitution request for an absent teacher') }}
            </p>
        </div>

        <div class="bg-white rounded-lg shadow-md p-6">
            <form method="POST" action="{{ route('substitutions.requests.store') }}">
                @csrf
                ...
                <div class="mt-6">
                    ...
                    <textarea name="notes" id="notes" rows="4"
                              ...
                              placeholder="{{ __('Optional instructions for the substitute teacher...') }}">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
