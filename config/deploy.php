...
return [
    ...
    | test server .env and the GitHub Actions secret TEST_DEPLOY_WEBHOOK_SECRET
    | (environment: test). Leave empty to disable the endpoint (404). Must be at
    | least 16 characters when enabled.
    |
    */
    ...
    */
    ...
    'test_allowed_hosts' => array_values(array_filter(array_map(
        'trim',
        explode(',', (string) env('TEST_DEPLOY_ALLOWED_HOSTS', 'test.akuru.edu.mv')),
    ))),
    ...
    |--------------------------------------------------------------------------
    |
    | cPanel account home for the test server. The deploy log is written to
    | <home>/self-update-test.log.
    |
    */

    'test_home_path' => env('TEST_DEPLOY_HOME', '/home/akuruedu'),
